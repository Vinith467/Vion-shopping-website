...
const ProductShowcaseSection = ({ videoUrl, content, showFloatingButtons, onBack, onWishlist, isWishlisted }) => {
  const sectionRef = useRef(null);
      ...
      <div ref={sectionRef} className="sticky top-0 w-full h-screen bg-[#FDFBF7] dark:bg-[#0A0A0A] overflow-hidden flex items-center transition-colors duration-500">
        ...
        {showFloatingButtons && (
          <div className="absolute top-0 left-0 right-0 z-50 p-6 md:p-10 flex justify-between items-start pointer-events-none">
            <button onClick={onBack} className="p-3 bg-black/20 hover:bg-black/40 backdrop-blur-md border border-white/20 text-white rounded-full transition-colors shadow-sm pointer-events-auto">
              <ArrowLeft size={20} />
            </button>
            <button 
              onClick={onWishlist}
...
export default function ProductDetailScreen() {
  ...
  return (
    <div className="bg-[#FDFBF7] dark:bg-[#0A0A0A] min-h-[100dvh] w-full font-sans pb-24 lg:pb-0 transition-colors duration-500 relative">
      ...
      <div className={`w-full relative bg-black overflow-hidden ${(!product?.marketing_content?.banner_image_url && !product?.marketing_content?.banner_video_url) ? 'h-[100dvh]' : 'h-[100dvh] md:h-auto md:aspect-video'}`}>
        ...
        <div className="relative z-10 w-full h-full flex items-center justify-center">
          {product?.marketing_content?.banner_video_url ? (
            <video 
              ...
              loop playsInline 
              className="w-full h-full object-cover md:h-auto md:object-contain block" 
            />

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-[#151515] transition-colors duration-500 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            return (
              <div key={activity.id} className={`flex items-center justify-between p-4 ${index !== activities.length -1 ? 'border-b border-gray-50' : ''}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-[#F5F0E8]">{activity.device}</h4>
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-100">
          <button className="w-full bg-white dark:bg-[#151515] transition-colors duration-500 border border-gray-200 hover:bg-gray-100 text-gray-700 font-bold py-2.5 rounded-xl transition-all shadow-sm text-sm">
